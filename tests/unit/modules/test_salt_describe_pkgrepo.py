...
import pytest
import saltext.salt_describe.modules.salt_describe_pkgrepo as salt_describe_pkgrepo_module
import yaml
...
def test_pkgrepo_redhat():
    ...
    with patch.dict(
        salt_describe_pkgrepo_module.__salt__,
        {"pkg.list_repos": MagicMock(return_value=pkgrepo_list)},
    ):
        with patch(
            "salt.utils.minions.get_minion_data", MagicMock(return_value=mock_minion_data)
        ) as minion_data_mock:
            with patch.object(salt_describe_pkgrepo_module, "generate_files") as generate_mock:
                assert "Generated SLS file locations" in salt_describe_pkgrepo_module.pkgrepo()
...
def test_pkgrepo_permission_denied(minion_opts, caplog, perm_denied_error_log):
    ...
    with patch.dict(
        ...
        with patch(
            ...
            with patch.dict(salt_describe_pkgrepo_module.__opts__, minion_opts):
                with patch.object(PosixPath, "mkdir", side_effect=PermissionError), patch.object(
                    ...
                    with caplog.at_level(logging.WARNING):
                        ret = salt_describe_pkgrepo_module.pkgrepo()
                        assert not ret
