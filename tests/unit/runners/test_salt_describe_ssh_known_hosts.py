...
import saltext.salt_describe.runners.salt_describe_ssh_known_hosts as salt_describe_ssh_known_hosts_runner
...
def configure_loader_modules():
    return {
        salt_describe_ssh_known_hosts_runner: {
...
def test_ssh_virtual():
    ret = salt_describe_ssh_known_hosts_runner.__virtual__()
    assert ret == "describe"
...
def test_ssh_user_keys():
    ...
    with patch.dict(
        salt_describe_ssh_known_hosts_runner.__salt__,
        {"salt.execute": MagicMock(return_value=ssh_known_hosts)},
    ):
        with patch.object(salt_describe_ssh_known_hosts_runner, "generate_files") as generate_mock:
            ret = salt_describe_ssh_known_hosts_runner.ssh_known_hosts("minion")
    ...
    with patch.dict(
        ...
        with patch.object(salt_describe_ssh_known_hosts_runner, "generate_files") as generate_mock:
            ...
            assert (
                ...
                in salt_describe_ssh_known_hosts_runner.ssh_known_hosts("minion")
            )
            generate_mock.assert_called_with(
                {}, "minion", ssh_known_hosts_sls, sls_name="ssh_known_hosts", config_system="salt"
            )
...
def test_ssh_known_hosts_permission_denied(caplog, minion_opts, perm_denied_error_log):
    ...
    with patch.dict(
        ...
        with patch.dict(salt_describe_ssh_known_hosts_runner.__opts__, minion_opts):
            with (
...
def test_ssh_known_hosts_parse_salt_ret_false(minion_opts):
    ...
    with patch.object(
        salt_describe_ssh_known_hosts_runner, "parse_salt_ret"
...
"""
    ...
    with patch.dict(
        ...
        with patch.object(salt_describe_ssh_known_hosts_runner, "generate_files") as generate_mock:
            assert (
                "Generated SLS file locations"
                in salt_describe_ssh_known_hosts_runner.ssh_known_hosts(
    ...
    with patch.dict(
        ...
        with patch.object(salt_describe_ssh_known_hosts_runner, "generate_files") as generate_mock:
            assert (
                "Generated SLS file locations"
                in salt_describe_ssh_known_hosts_runner.ssh_known_hosts(
                    "minion", config_system="ansible", hosts="testgroup"
                )
            )
            generate_mock.assert_called_with(
                {},
